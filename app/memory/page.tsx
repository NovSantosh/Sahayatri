...
export default function Memory() {
  ...
  return (
    <div style={{minHeight: '100vh', background: t.pageBg, fontFamily: 'Inter, -apple-system, sans-serif', paddingBottom: '40px', transition: 'background 0.3s ease'}}>
      ...
      {/* Delete modal */}
      {confirmDelete && (
        <div style={{position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '24px', backdropFilter: 'blur(4px)'}}>
          <div style={{background: t.cardBg, borderRadius: '24px', padding: '28px 24px', width: '100%', maxWidth: '380px', border: `1px solid ${t.border}`}}>
            <div style={{width: '52px', height: '52px', borderRadius: '50%', background: brand.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px'}}>
              <TrashIcon size={24} color={brand.primary} strokeWidth={2}/>
            </div>
            <h3 style={{fontSize: '17px', fontWeight: 700, color: t.text1, textAlign: 'center', marginBottom: '8px'}}>Delete this moment?</h3>
      ...
      <div style={{background: t.headerBg, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', padding: '52px 20px 0', borderBottom: `1px solid ${t.border}`, position: 'sticky', top: 0, zIndex: 50, transition: 'background 0.3s ease'}}>
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px'}}>
          <div>
            <h1 style={{fontSize: '24px', fontWeight: 900, color: t.text1, letterSpacing: '-0.8px', transition: 'color 0.3s ease'}}>Memory</h1>
      ...
      {/* ── COMPOSE ── */}
      {showForm && (
        <div style={{margin: '16px 16px 0', ...card, padding: '20px'}}>
          {error && (
            <div style={{background: brand.primaryLight, border: `1px solid ${brand.primaryBorder}`, borderRadius: '10px', padding: '10px 14px', marginBottom: '12px', fontSize: '13px', color: brand.primary, fontWeight: 600}}>
              {error}
            </div>
          )}
          {session?.user && (
            <div style={{display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px'}}>
              <div style={{width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #DC143C, #A50E2D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '13px', fontWeight: 800, flexShrink: 0}}>
                {initials(session.user.name || '')}
              </div>
              <div>
                <p style={{fontSize: '14px', fontWeight: 700, color: t.text1}}>{session.user.name}</p>
                <p style={{fontSize: '11px', color: t.text3}}>Sharing a moment</p>
              </div>
            </div>
          )}
              ...
              {imagePreview.map((url, i) => (
                <div key={i} style={{position: 'relative'}}>
                  ...
                  <button onClick={() => { imagesRef.current = imagesRef.current.filter((_, j) => j !== i); setImagePreview([...imagesRef.current]) }}
                    style={{position: 'absolute', top: '-6px', right: '-6px', width: '22px', height: '22px', borderRadius: '50%', background: brand.primary, border: 'none', color: 'white', fontSize: '13px', cursor: 'pointer', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>×</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '14px', flexWrap: 'wrap', gap: '10px'}}>
            <div style={{display: 'flex', gap: '6px', flexWrap: 'wrap'}}>
              {['Care moment', 'Service story', 'Community'].map((cat) => (
                <button key={cat} onClick={() => setCategory(cat)}
                  style={{padding: '5px 12px', borderRadius: '9999px', border: `1.5px solid ${category === cat ? brand.primary : t.border}`, background: category === cat ? brand.primaryLight : 'transparent', color: category === cat ? brand.primary : t.text3, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.2s ease'}}>
                  {cat}
                </button>
              ))}
            </div>
            <div style={{display: 'flex', gap: '8px', alignItems: 'center'}}>
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={uploading || imagePreview.length >= 4}
                style={{padding: '8px 14px', background: t.inputBg, border: `1px solid ${t.border}`, borderRadius: '12px', fontSize: '12px', fontWeight: 600, color: t.text2, cursor: 'pointer', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', gap: '6px'}}>
                <CameraIcon size={14} color={t.text2} strokeWidth={2}/>
                {uploading ? 'Uploading…' : 'Photo'}
              </button>
              <button onClick={handlePost} disabled={posting || !content.trim()}
                style={{padding: '10px 20px', background: !content.trim() ? t.inputBg : 'linear-gradient(135deg, #DC143C, #A50E2D)', border: 'none', borderRadius: '12px', fontSize: '14px', fontWeight: 800, cursor: !content.trim() ? 'not-allowed' : 'pointer', fontFamily: 'Inter, sans-serif', color: !content.trim() ? t.text3 : 'white', boxShadow: content.trim() ? '0 4px 16px rgba(220,20,60,0.3)' : 'none', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s ease'}}>
                <SendIcon size={14} color={!content.trim() ? t.text3 : 'white'} strokeWidth={2}/>
      ...
      <div style={{padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px'}}>
        ...
        {loading && (
          <div style={{textAlign: 'center', padding: '60px 20px'}}>
            ...
            <p style={{fontSize: '13px', color: t.text3}}>Loading moments…</p>
          </div>
        )}
        ...
        {!loading && filteredPosts.length === 0 && (
          <div style={{...card, padding: '48px 24px', textAlign: 'center'}}>
            <div style={{width: '64px', height: '64px', borderRadius: '50%', background: brand.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px'}}>
              <HeartIcon size={32} color={brand.primary} strokeWidth={1.5}/>
            </div>
            <h3 style={{fontSize: '17px', fontWeight: 700, color: t.text1, marginBottom: '8px'}}>No moments yet</h3>
            <p style={{fontSize: '13px', color: t.text3, marginBottom: '20px'}}>Be the first to share something meaningful.</p>
        ...
        {filteredPosts.map((post) => {
          ...
          return (
            <div key={post._id} style={{...card, overflow: 'hidden'}}>

              {/* Post header */}
              <div style={{display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 16px 12px'}}>
                <div style={{width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(135deg, #DC143C, #A50E2D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '15px', flexShrink: 0, boxShadow: '0 4px 12px rgba(220,20,60,0.25)'}}>
                  {initials(post.authorName)}
                </div>
                <div style={{flex: 1}}>
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                    <p style={{fontSize: '15px', fontWeight: 700, color: t.text1, transition: 'color 0.3s ease'}}>{post.authorName}</p>
                    <div style={{padding: '2px 8px', borderRadius: '9999px', background: catStyle.bg}}>
                      <span style={{fontSize: '10px', fontWeight: 700, color: catStyle.color}}>{catStyle.label}</span>
                    </div>
                  </div>
                  <p style={{fontSize: '11px', color: t.text3, marginTop: '2px'}}>{timeAgo(post.createdAt)} ago</p>
                </div>
                {isOwn && (
                  <button onClick={() => setConfirmDelete(post._id)}
                    style={{width: '32px', height: '32px', borderRadius: '10px', background: brand.primaryLight, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'}}>
                    <TrashIcon size={14} color={brand.primary} strokeWidth={2}/>
                  </button>
                )}
              </div>
              ...
              {/* Content */}
              {post.content && (
                <p style={{fontSize: '15px', color: t.text1, lineHeight: 1.7, padding: '0 16px 14px', transition: 'color 0.3s ease'}}>{post.content}</p>
              )}
              ...
              {/* Actions */}
              <div style={{display: 'flex', borderTop: `1px solid ${t.border}`, padding: '4px 8px'}}>
                <button onClick={() => handleLike(post._id)} disabled={likingIds.includes(post._id)}
                  style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px', border: 'none', background: 'transparent', borderRadius: '12px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.15s ease'}}>
                  <HeartIcon size={18} color={post.liked ? brand.primary : t.text3} strokeWidth={2} filled={post.liked}/>
                  {post.likes > 0 && <span style={{fontSize: '13px', fontWeight: 600, color: post.liked ? brand.primary : t.text3}}>{post.likes}</span>}
                </button>
                <button onClick={() => setOpenComments(prev => prev.includes(post._id) ? prev.filter(id => id !== post._id) : [...prev, post._id])}
                  style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px', border: 'none', background: 'transparent', borderRadius: '12px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.15s ease'}}>
                  <CommentIcon size={18} color={isCommenting ? brand.primary : t.text3} strokeWidth={2}/>
                  {post.comments?.length > 0 && <span style={{fontSize: '13px', fontWeight: 600, color: isCommenting ? brand.primary : t.text3}}>{post.comments.length}</span>}
                </button>
                <button style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px', border: 'none', background: 'transparent', borderRadius: '12px', cursor: 'pointer', fontFamily: 'Inter, sans-serif'}}>
                  <ShareIcon size={18} color={t.text3} strokeWidth={2}/>
                </button>
              </div>

              {/* Comments */}
              {isCommenting && (
                <div style={{borderTop: `1px solid ${t.border}`, padding: '14px 16px', background: t.inputBg, transition: 'background 0.3s ease'}}>
                  {post.comments?.length > 0 && (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '14px'}}>
                      {post.comments.map((c: any, i: number) => (
                        <div key={i} style={{display: 'flex', gap: '10px'}}>
                          <div style={{width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #DC143C, #A50E2D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '10px', fontWeight: 800, flexShrink: 0}}>
                            {initials(c.authorName)}
                          </div>
                          <div style={{flex: 1, background: t.cardBg, borderRadius: '12px', padding: '10px 12px', border: `1px solid ${t.border}`}}>
                            <p style={{fontSize: '12px', color: t.text1, fontWeight: 700, marginBottom: '3px'}}>{c.authorName}</p>
                            <p style={{fontSize: '13px', color: t.text2, lineHeight: 1.5}}>{c.text}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {!post.comments?.length && (
                    <p style={{fontSize: '13px', color: t.text3, textAlign: 'center', marginBottom: '12px'}}>No comments yet. Be the first.</p>
                  )}
                  <div style={{display: 'flex', gap: '8px', alignItems: 'center'}}>
                    <div style={{width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #DC143C, #A50E2D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '10px', fontWeight: 800, flexShrink: 0}}>
                      {initials(session?.user?.name || '')}
                    </div>
                    <input value={commentText[post._id] || ''}
                      onChange={(e) => setCommentText(prev => ({ ...prev, [post._id]: e.target.value }))}
                      onKeyDown={(e) => e.key === 'Enter' && handleComment(post._id)}
                      placeholder="Write a comment…"
                      style={{flex: 1, background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: '9999px', padding: '10px 16px', fontSize: '13px', color: t.text1, outline: 'none', fontFamily: 'Inter, sans-serif', transition: 'background 0.3s ease'}}/>
                    <button onClick={() => handleComment(post._id)} disabled={!commentText[post._id]?.trim() || submittingComment.includes(post._id)}
                      style={{width: '36px', height: '36px', borderRadius: '50%', background: commentText[post._id]?.trim() ? 'linear-gradient(135deg, #DC143C, #A50E2D)' : t.inputBg, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: commentText[post._id]?.trim() ? 'pointer' : 'not-allowed', flexShrink: 0, boxShadow: commentText[post._id]?.trim() ? '0 4px 12px rgba(220,20,60,0.3)' : 'none', transition: 'all 0.2s ease'}}>
                      <SendIcon size={14} color={commentText[post._id]?.trim() ? 'white' : t.text3} strokeWidth={2}/>
                    </button>
                  </div>
                </div>
              )}
        ...
        {!loading && filteredPosts.length > 0 && (
          <div style={{textAlign: 'center', padding: '24px 20px'}}>
            <p style={{fontSize: '15px', fontWeight: 700, color: t.text1, marginBottom: '4px'}}>You are all caught up.</p>
            <p style={{fontSize: '13px', color: t.text3}}>Now go call someone you love.</p>
          </div>
        )}
      </div>
