...
export default function Memory() {
  ...
  const handlePost = async () => {
    ...
    if (!session?.user) {
      setError('Please log in to share a moment')
      return
    }
    setPosting(true)
    setError('')
    ...
    <div style={{minHeight: '100vh', background: '#F5F6F8', fontFamily: 'sans-serif', paddingBottom: '80px'}}>
      ...
      {showForm && (
        <div style={{margin: '14px 16px 0', background: 'white', borderRadius: '20px', border: '1px solid #E9EAEC', padding: '16px'}}>
          {!session?.user && (
            <div style={{background: '#FEF2F2', border: '1px solid rgba(220,20,60,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '10px', fontSize: '13px', color: '#DC143C', fontWeight: 600}}>
              Please <a href="/login" style={{color: '#DC143C', textDecoration: 'underline'}}>log in</a> to share a moment.
            </div>
          )}
          {error && (
            <div style={{background: '#FEF2F2', border: '1px solid rgba(220,20,60,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '10px', fontSize: '13px', color: '#DC143C', fontWeight: 600}}>{error}</div>
          )}
          {session?.user && (
            <div style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px'}}>
              <div style={{width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #DC143C, #A50E2D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px', fontWeight: 800}}>
          ...
          <textarea
            ...
            placeholder="What happened today? Share a real moment..."
            rows={4}
          ...
          <div style={{display: 'flex', gap: '8px', marginTop: '10px', marginBottom: '12px'}}>
            {['Care moment', 'Service story', 'Community'].map((cat) => (
              <button key={cat} onClick={() => setCategory(cat)} style={{padding: '5px 10px', borderRadius: '20px', border: `1.5px solid ${category === cat ? '#DC143C' : '#E9EAEC'}`, background: category === cat ? 'rgba(220,20,60,0.08)' : 'white', color: category === cat ? '#DC143C' : '#9CA3AF', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'sans-serif'}}>
                {cat}
              </button>
            ))}
          </div>
          <div style={{display: 'flex', gap: '8px'}}>
            <button onClick={() => setShowForm(false)} style={{flex: 1, padding: '11px', background: '#F5F6F8', border: 'none', borderRadius: '12px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'sans-serif', color: '#6B7280'}}>Cancel</button>
      ...
      <div style={{padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '14px'}}>
        ...
        {posts.map((post) => (
          <div key={post._id} style={{background: 'white', borderRadius: '20px', border: '1px solid #E9EAEC', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)'}}>
            ...
            <div style={{padding: '4px 16px 16px', fontSize: '14px', color: '#374151', lineHeight: 1.7}}>
              {post.content}
            </div>
